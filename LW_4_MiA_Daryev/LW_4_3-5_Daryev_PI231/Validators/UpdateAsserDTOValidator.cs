...
{
    public class UpdateAsserDTOValidator : AbstractValidator<UpdateAssetDTO>
    {
        public UpdateAsserDTOValidator()
        {
            RuleFor(asset => asset.Name)
                   .NotEmpty().WithMessage("Name is required.")
                   .MaximumLength(100).WithMessage("Name cannot exceed 100 characters.");

            RuleFor(asset => asset.Type).NotEmpty().WithMessage("Device type is required.")
            ...
            RuleFor(asset => asset.Status).NotEmpty().WithMessage("Status is required.")
